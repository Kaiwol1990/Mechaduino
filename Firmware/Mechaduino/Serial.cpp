...
void parameterEdit(int arg_cnt, char **args) {
  ...
  if (set_param) {
    ...
    Kff = return_float_argument(args, arg_cnt, "-Kff", Kff, 0, 10);
    Kacc = return_float_argument(args, arg_cnt, "-Kacc", Kacc, 0, 10);
    ...
    error_LPF = return_float_argument(args, arg_cnt, "-error_LPF", error_LPF, 100, 5000);
    ...
    step_add = 1024 / microstepping;
    uMAX = ((512.0 * iMAX * 10 * rSense) / (1000 * 3.3));
    ITerm_max = (uMAX * 1024) + 0.5;
    error_LPFa = ((2048.0 * exp(error_LPF * -2.0 * 3.14159283 / Fs)) + 0.5); // z = e^st pole mapping
    error_LPFb = 2048 - error_LPFa;
...
void step_response(int arg_cnt, char **args) {
  ...
  int response_steps = return_float_argument(args, arg_cnt, "-s", 1000, 200, 10000);
...
void set_streaming(int arg_cnt, char **args) {
  ...
  frequency = return_float_argument(args, arg_cnt, "-f", frequency, 1, 5000);
...
void streaming_handler() {
  ...
  uint16_t buffer_length = fifo_counter - serial_counter;

  if (streaming && (buffer_length > 0)) {

    byte serial_position = mod(serial_counter, 49);

    SerialUSB.print("s,");
    for (int i = 0; i < 8; i++) {
      if (stream_variables[i]) {
